...
unmelt <- function(long_df, row_col, col_col, val_col){
  ...
  wide_df <- pivot_wider(long_df, id_cols=all_of(c(row_col)), names_from=all_of(col_col), values_from=all_of(val_col)) %>% as.data.frame()
  #wide_df <- pivot_wider(long_df, names_from=all_of(col_col), values_from=all_of(val_col)) %>% as.data.frame()
